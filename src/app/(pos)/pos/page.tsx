...
export default async function PosPage() {
  ...
  return (
    <PosTerminal
      ...
      customerDisplaySessions={features.customer_display ? customerDisplaySessions : []}
      deviceManagementEnabled={context.organization.device_management_enabled}
      diningOptions={features.dining ? diningOptions : []}
      discounts={discounts}
      ...
      key={`${activeShift?.id ?? "shift-closed"}:${openTickets.map((ticket) => `${ticket.id}:${ticket.updatedAt}`).join(",")}`}
      loyaltyProgram={features.loyalty ? loyaltyProgram : null}
      organizationName={context.organization.name}
      openTickets={features.open_tickets ? openTickets : []}
      ...
      stores={stores}
      taxRates={taxRates}
      ticketAssignees={features.open_tickets ? ticketAssignees : []}
      ticketTemplates={features.open_tickets ? ticketTemplates : []}
      timeClockEntry={features.time_clock ? timeClockEntry : null}
