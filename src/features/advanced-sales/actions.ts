...
export type AdvancedSalesResult = { ok: true; message: string } | { ok: false; message: string };
...
async function canManage() {
  ...
  return hasPermission(context, "products.manage") ? context : null;
}

async function canManageFeature(feature: "dining" | "modifiers" | "open_tickets") {
  const context = await canManage();
  return context?.features[feature] ? context : null;
}

export async function createDiscountAction(input: unknown): Promise<AdvancedSalesResult> {
  const context = await canManage();
  if (!context) return { ok: false, message: "You do not have permission to configure advanced sales." };
  ...
  const { error } = await (await database()).from("discounts").insert(record);
  ...
  refresh(); return { ok: true, message: "Discount created." };
}
...
export async function createTaxRateAction(input: unknown): Promise<AdvancedSalesResult> {
  ...
  refresh(); return { ok: true, message: "Tax rate created." };
}

export async function createDiningOptionAction(input: unknown): Promise<AdvancedSalesResult> {
  const context = await canManageFeature("dining");
  if (!context) return { ok: false, message: "Dining is disabled or you do not have permission to configure it." };
  ...
  refresh(); return { ok: true, message: "Dining option created." };
}

export async function createTicketTemplateAction(input: unknown): Promise<AdvancedSalesResult> {
  const context = await canManageFeature("open_tickets");
  if (!context) return { ok: false, message: "Open tickets are disabled or you do not have permission to configure them." };
...
export async function createModifierGroupAction(input: unknown): Promise<AdvancedSalesResult> {
  const context = await canManageFeature("modifiers");
  if (!context) return { ok: false, message: "Modifiers are disabled or you do not have permission to configure them." };
  ...
  refresh(); return { ok: true, message: "Modifier group created." };
}
