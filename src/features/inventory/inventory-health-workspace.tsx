...
export function InventoryControlTower({ healthHref, issues, recentActivity, stores, transfers, transfersHref }: {
  healthHref: string;
  ...
  return <section aria-labelledby="inventory-control-tower-title" className="space-y-4">
    <div><h2 className="text-lg font-semibold" id="inventory-control-tower-title">Stock health</h2><p className="mt-1 text-sm text-muted-foreground">Attention-first signals derived from current stock, completed counts, and open inventory documents.</p></div>
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>Multi-store stock</CardTitle><CardDescription>Issues, open transfers, and latest completed physical count remain store-specific.</CardDescription></CardHeader><CardContent>{stores.length ? <div className="overflow-x-auto rounded-xl border"><table className="w-full min-w-[34rem] text-sm"><thead className="bg-muted/30 text-left text-xs text-muted-foreground"><tr><th className="px-3 py-2">Store</th><th className="px-3 py-2">Stock issues</th><th className="px-3 py-2">Transfers</th><th className="px-3 py-2">Last count</th></tr></thead><tbody className="divide-y">{stores.map((store) => <tr key={store.storeId}><td className="px-3 py-2 font-medium">{store.name}</td><td className="px-3 py-2">{store.issueCount ? `${store.issueCount} issue${store.issueCount === 1 ? "" : "s"}` : "Healthy"}</td><td className="px-3 py-2">{store.openTransferCount || "—"}</td><td className="px-3 py-2">{formatRelativeDays(store.daysSinceLastCount)}</td></tr>)}</tbody></table></div> : <EmptyState text="No authorized stores are available." />}</CardContent></Card>
...
export function InventoryHealthWorkspace({ initialSeverity = "all", issues, stores }: {
  ...
  return <section aria-labelledby="stock-health-page-title" className="space-y-4">
    <div><h2 className="text-lg font-semibold" id="stock-health-page-title">Stock Health</h2><p className="mt-1 text-sm text-muted-foreground">Review situations that may be wrong or need investigation. This page never changes stock automatically.</p></div>
...
function TowerMetric({ href, label, value }: { href: string; label: string; value: number }) { return <Link href={href}><Card className="h-full transition-colors hover:border-primary/40"><CardContent className="p-4"><p className="text-sm text-muted-foreground">{label}</p><p className="mt-1 text-3xl font-semibold">{value}</p></CardContent></Card></Link>; }
...
function formatRelativeDays(days: number | null) { if (days === null) return "Never"; return days === 0 ? "Today" : days === 1 ? "Yesterday" : `${days} days ago`; }
