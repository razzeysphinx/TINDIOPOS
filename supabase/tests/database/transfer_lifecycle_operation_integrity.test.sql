...
select public.record_inventory_adjustment_v3(
  (select organization_id from transfer_operation_context), (select source_store_id from transfer_operation_context), (select product_id from transfer_operation_context), 4, 'SEED', 'Seed transfer source', gen_random_uuid()
);
update transfer_operation_context
